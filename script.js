...
var TYPES={strong:{color:'#8b5a2b',kind:'strong'},fast:{color:'#cf2e2e',kind:'fast'},hybrid:{kind:'hybrid'},growing:{color:'#f2d200',kind:'growing'},laser:{color:'#1abc9c',kind:'laser'},gravitron:{color:'#9b59b6',kind:'gravitron'},duplicator:{color:'#ff66cc',kind:'duplicator'}};
...
function baseBallFor(type){var isSpeedy=(type==='fast'||type==='hybrid');var dir=(Math.random()<0.5?-1:1);var startX=laneW/2+(Math.random()*40-20);var t=TYPES[type]||TYPES.strong;var baseVx=isSpeedy?2*dir:3*dir;var b={type:t.kind,x:startX,y:44,r:14,color:(t.color||'#000'),vx:baseVx,vy:0,g:0.45,damage:1,dirX:(baseVx>=0?1:-1),dirY:1,growRatePS:(type==='growing'?2:0),finished:false,timeToFinish:null,popping:false,popProg:0,popped:false};
...
var lanes=[],isRunning=false,isPaused=false,winner=null,startTime=0,raf,lastTime=0,dtSec=0;
...
function applyAndStart(){config.left.top=clampHP(hpTop1&&hpTop1.value);config.left.mid=clampHP(hpMid1&&hpMid1.value);config.left.bot=clampHP(hpBot1&&hpBot1.value);config.separate=!!(separateChk&&separateChk.checked);if(config.separate){config.right.top=clampHP(hpTop2&&hpTop2.value);config.right.mid=clampHP(hpMid2&&hpMid2.value);config.right.bot=clampHP(hpBot2&&hpBot2.value)}else{config.right.top=config.left.top;config.right.mid=config.left.mid;config.right.bot=config.left.bot}config.left.type=(typeLeftSel&&typeLeftSel.value)||config.left.type||'strong';config.right.type=(typeRightSel&&typeRightSel.value)||config.right.type||'fast';config.configured=true;showMenu(false);reset();start()}
...
function intersectRayRect(x0,y0,dx,dy,rx,ry,rw,rh){var tmin=-Infinity,tmax=Infinity;if(dx!==0){var tx1=(rx-x0)/dx,tx2=(rx+rw-x0)/dx;var a=Math.min(tx1,tx2),b=Math.max(tx1,tx2);tmin=Math.max(tmin,a);tmax=Math.min(tmax,b)}else if(x0<rx||x0>rx+rw){return null}if(dy!==0){var ty1=(ry-y0)/dy,ty2=(ry+rh-y0)/dy;var c=Math.min(ty1,ty2),d=Math.max(ty1,ty2);tmin=Math.max(tmin,c);tmax=Math.min(tmax,d)}else if(y0<ry||y0>ry+rh){return null}if(tmax<0||tmin>tmax){return null}return tmin>=0?tmin:tmax}
function raycastBlocks(b,L){var dx=Math.cos(b.laserAngle),dy=Math.sin(b.laserAngle);var bestT=Infinity,bestIdx=-1;for(var i=0;i<L.blocks.length;i++){var blk=L.blocks[i];if(blk.broken)continue;var t=intersectRayRect(b.x,b.y,dx,dy,blk.x,blk.y,blk.w,blk.h);if(t!==null&&t>=0&&t<bestT){bestT=t;bestIdx=i}}var endX,endY;if(bestIdx>=0){endX=b.x+dx*bestT;endY=b.y+dy*bestT}else{var tBox=intersectRayRect(b.x,b.y,dx,dy,BORDER,BORDER,laneW-2*BORDER,laneH-2*BORDER);if(!(tBox>0))tBox=800;endX=b.x+dx*tBox;endY=b.y+dy*tBox}return{x1:b.x,y1:b.y,x2:endX,y2:endY,hitIdx:bestIdx}}
function updateLasers(L){
  L.laserRays.length=0;
  for(var i=0;i<L.balls.length;i++){
    var b=L.balls[i];
    if(b.type!=='laser' || b.popped) continue;
    var w=Math.abs(b.laserAngVel||1),T=Math.PI*2;
    b.laserAngle+=w*dtSec;
    if(b.laserAngle>=T){b.laserAngle-=T}
    var ray=raycastBlocks(b,L);
    L.laserRays.push(ray);
    ...
    if(hit>=0){
      ...
      while(b.laserTimer>=b.laserInterval){
        ...
        if(!blk.broken){
          blk.hp=Math.max(0,blk.hp-(b.laserDamage||1));
          if(blk.hp===0){blk.broken=true}
          b.laserHits=(b.laserHits||0)+1;
        }else{
          break;
        }
      }
    }else{
      b.lastLaserBlockIdx=-1; b.laserTimer=0;
    }
  }
}
function duplicateBall(L,idx){
  ...
  var s=Math.max(3,Math.hypot(b.vx||0,b.vy||0));
  var off=b.r*1.25;
  var nx=b.x+Math.cos(ang)*off;
  var ny=b.y+Math.sin(ang)*off;
    ...
    for(var bi=0;bi<L.balls.length;bi++){
      ...
      if(!(b.finished||b.popping)){
        if(b.type==='fast'||b.type==='hybrid'){
          b.spd=b.spd||Math.max(2,Math.abs(b.vx));
          var _a=b.spdAccel||0.008,_k=b.spdAccelK||0.0013333333;
          b.spd+=_a+_k*b.spd;
          b.vx=(b.dirX>=0?1:-1)*b.spd;
        }
        if(b.type==='gravitron'){ var k=(b.g>=7.5?(b.gK||0):0); b.g += (b.gIncPS||0)*dtSec + k*b.g*dtSec; }
        if(b.type==='growing'){
          b.r+=(b.growRatePS||0)*dtSec;
        }
        if(b.type==='duplicator'){
          b.dupTimer=(b.dupTimer||0)+dtSec;
          if(b.dupTimer>=(b.dupEvery||5)){
            duplicateBall(L,bi);
          }
        }
        var tsX=TIME_SCALE,tsY=TIME_SCALE;
        b.x+=b.vx*tsX;
        b.y+=b.vy*tsY;
        b.vy+=b.g*tsY;
        ...
        if(b.y+b.r>=s.y && b.vy>0){
          b.y=s.y-b.r;
          if(s.idx>=0){
            var blk=L.blocks[s.idx];
            if(!blk.broken){
              var dmg=(b.type==='strong'||b.type==='hybrid')?(b.damage||1):1;
              blk.hp=Math.max(0,blk.hp-dmg);
              if(b.type==='strong'||b.type==='hybrid'){ b.damage=(b.damage||1)+1 }
              if(b.type==='fast'||b.type==='hybrid'){
                b.vy=-Math.max(bounce(b),(b.spd||Math.abs(b.vx)));
              }else{
                b.vy=-bounce(b);
              }
            ...
            if(winner===null){
              winner=li; winnerEl.textContent=li===0?'Победитель: левая коробка 🏁':'Победитель: правая коробка 🏁';
...
function refreshHUD(){
  function textForLane(L){
    if(!L||!L.balls||L.balls.length===0) return '—';
    var alive=L.balls.filter(function(x){return !x.popped});
    var b=alive[0]||L.balls[0];
    if(b.type==='fast') return 'Скорость: '+Math.abs(b.vx).toFixed(2);
    if(b.type==='hybrid') return 'Урон: '+(b.damage||1)+' | Скорость: '+Math.abs(b.vx).toFixed(2);
    if(b.type==='growing') return 'Радиус: '+b.r.toFixed(1);
    if(b.type==='laser') return 'Лазер: '+(b.laserDamage||1).toFixed(2)+' | t '+(b.laserInterval||0).toFixed(3);
    if(b.type==='gravitron') return 'Гравитация: '+(b.g||0).toFixed(2);
    if(b.type==='duplicator') return 'Клонов: '+alive.length;
    return 'Урон: '+(b.damage||1);
  }
...
function loop(){
  ...
  if(winner===null){
    ...
    for(var li=0;li<lanes.length;li++){
      ...
      for(var bi=0; bi<L.balls.length; bi++){
        var b=L.balls[bi];
        if(b&&b.popping&&!b.popped){ anyPopping=true; b.popProg+=0.06; if(b.popProg>=1){ b.popProg=1; b.popped=true; b.popping=false; } }
      }
    }
...
function drawLane(L){
  if(!L||!L.blocks) return; var ctx=L.ctx; ctx.clearRect(0,0,laneW,laneH); ctx.fillStyle='#ffffff'; ctx.fillRect(0,0,laneW,START_PAD);
  var primaryBall=(L.balls&&L.balls[0])||null; var tint=tintColor(primaryBall);
  for(var i=0;i<L.blocks.length;i++){
    var bl=L.blocks[i]; if(bl.broken) continue; var base={r:255,g:255,b:255}; var ratio=bl.max>0?(1-(bl.hp/bl.max)):1; var t=Math.max(0,Math.min(1,ratio)); var col=mixRGB(base,tint,t); ctx.fillStyle=rgbToCss(col); ctx.strokeStyle='#000'; ctx.lineWidth=3; ctx.fillRect(bl.x,bl.y,bl.w,bl.h); ctx.strokeRect(bl.x,bl.y,bl.w,bl.h); ctx.fillStyle='#000'; ctx.font='900 44px Impact, Inter, system-ui, Arial'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(String(bl.hp),bl.x+bl.w/2,bl.y+bl.h/2);
  }
  drawFinish(ctx);
  if(L.laserRays){ for(var r=0;r<L.laserRays.length;r++){ var ray=L.laserRays[r]; if(!ray) continue; ctx.beginPath(); ctx.moveTo(ray.x1,ray.y1); ctx.lineTo(ray.x2,ray.y2); ctx.lineWidth=3; ctx.strokeStyle='#1abc9c'; ctx.stroke(); if(ray.hitIdx>=0){ ctx.beginPath(); ctx.arc(ray.x2,ray.y2,4,0,Math.PI*2); ctx.fillStyle='#1abc9c'; ctx.fill(); } } }
  for(var bi=0; bi<L.balls.length; bi++){
    var ball=L.balls[bi]; if(!ball || ball.popped) continue; if(ball.popping){ drawPop(ctx,ball) } else { drawBall(ctx,ball) }
  }
}
function drawBall(ctx,ball){
  if(ball.type==='hybrid'){
    var r=ball.r; ctx.beginPath(); ctx.moveTo(ball.x,ball.y); ctx.arc(ball.x,ball.y,r,Math.PI/2,Math.PI*1.5); ctx.closePath(); ctx.fillStyle='#8b5a2b'; ctx.fill(); ctx.beginPath(); ctx.moveTo(ball.x,ball.y); ctx.arc(ball.x,ball.y,r,-Math.PI/2,Math.PI/2); ctx.closePath(); ctx.fillStyle='#cf2e2e'; ctx.fill(); ctx.beginPath(); ctx.arc(ball.x,ball.y,r,0,Math.PI*2); ctx.lineWidth=2; ctx.strokeStyle='#000'; ctx.stroke(); return;
  }
